...
\paragraph{Modelo de datos relacional.}
...
\paragraph{Endpoints de la API REST.}
...
\paragraph{Políticas de origen cruzado (CORS).}
...
\paragraph{Autenticación, Autorización y Roles.}
...
\paragraph{Modularización y Desacoplamiento (Capa CRUD y Servicios).}
...
\paragraph{Nodo Estado Inicial.}
Es el nodo de entrada del grafo y realiza un análisis estructurado consolidado en una sola llamada de inferencia al LLM ligero (Llama 3.1 8B) utilizando salida estructurada de Pydantic (\texttt{with\_structured\_output}). Sus funciones principales son:
\begin{enumerate}
...
\paragraph{Nodo Recuperador: pipeline RAG.}
Este nodo ejecuta el pipeline de recuperación híbrida y compresión de información:
\begin{enumerate}
...
\paragraph{Nodo Búsqueda Web: recuperación externa fallback.}
...
\paragraph{Nodo Entrevistador: gestión de ambigüedades.}
...
\paragraph{Nodos Resolutores: respuestas especializadas.}
...
\paragraph{Docker Compose: orquestación de servicios.}
...
\paragraph{Configuración de red y variables de entorno.}
...
\paragraph{Adaptación para despliegue y optimización en producción.}
...
\paragraph{Evolución de la estrategia de fragmentación.}

El desarrollo de este proyecto evidenció que la estrategia de fragmentación de los documentos posee un impacto crítico en la precisión de las respuestas del sistema. De hecho, la estrategia de fragmentación evolucionó a lo largo de tres fases hasta consolidar una solución satisfactoria:
...
\paragraph{Configuración final: MarkdownTextSplitter.}
...
\paragraph{Primera aproximación: Pipeline RAG lineal (Versión 1).}

Al comenzar el desarrollo del agente, se optó por la aproximación más directa y sencilla: un flujo lineal clásico de generación aumentada por recuperación. Esta primera versión constaba únicamente de dos nodos principales conectados en serie:
\begin{itemize}
...
\paragraph{Tercera aproximación: Recuperación condicional (Versión 3).}
...
\paragraph{Consolidación del Análisis Inicial con Salida Estructurada.}
...
\paragraph{Prompt de análisis inicial estructurado.}
...
\paragraph{Prompt de suficiencia de información.}
...
\paragraph{Prompts de los nodos resolutores.}
...
\paragraph{Búsqueda Híbrida (Dense + Sparse) y RRF.}
...
\paragraph{Reranking y Compresión con Cohere.}
...
\paragraph{Ventana deslizante de mensajes.}

Para evitar exceder la ventana de contexto de los modelos de Groq, se implementó una ventana deslizante que envía al agente únicamente los últimos 5 mensajes de la conversación. El historial completo se almacena en la base de datos, pero solo el subconjunto reciente se utiliza para la inferencia.

\paragraph{Compresión de memoria con resumen.}
...
\paragraph{Prevención de saturación del contexto en el perfilado.}
...
\paragraph{Resolución dinámica de la conexión backend.}

Durante la puesta en producción en Render, el frontend experimentó desconexiones persistentes (\texttt{fetch failed} y errores de tipo 502 \textit{Bad Gateway}). Inicialmente se intentó configurar un proxy inverso mediante las directivas \texttt{rewrites} de Next.js en el servidor para enmascarar las llamadas al backend. Sin embargo, al tratarse de flujos continuos de eventos tipo SSE, el proxy de Next.js tendía a acumular el buffer de datos en memoria antes de mandarlo o daba fallos de resolución de DNS internos. 
...
\paragraph{Tratamiento amigable de fallos y límites de API (SSE).}
...
\paragraph{Fase 1: Modelos locales (Llama 3.1 8B).}
...
\paragraph{Fase 2: API de HuggingFace.}
...
\paragraph{LLM clasificador (Llama 3.1 8B Instant).}
...
\paragraph{LLM ligero (Llama 3.1 8B Instant).}
...
\paragraph{LLM principal (Llama 3.3 70B Versatile).}
...
Evalúa en una escala continua de 0.00 a 1.00 si la respuesta generada se basa exclusivamente en la información presente en el contexto recuperado, sin inventar datos. Una puntuación de 1.00 indica que toda la respuesta está fielmente soportada por los documentos.

\paragraph{Answer Relevancy.}
Evalúa en una escala continua de 0.00 a 1.00 si la respuesta del agente aborda adecuadamente la pregunta del usuario. Una puntuación de 1.00 indica que la respuesta responde perfectamente y de forma completa a lo preguntado.
