...
Para evitar que el asistente se comporte de manera impersonal, desarrollé un mecanismo de \textbf{perfilado dinámico y memoria persistente} que se ejecuta en segundo plano.

En la base de datos PostgreSQL, agregué una columna \texttt{perfil\_metadata} (tipo texto con estructura JSON) en la tabla de usuarios. Al finalizar cada interacción de chat, en lugar de ralentizar el flujo de respuesta del usuario, ejecuto una tarea en segundo plano mediante \texttt{BackgroundTasks} de FastAPI:
\begin{enumerate}
...
Para visualizar respuesta final, utilizo el componente \texttt{ReactMarkdown}, mapeando elementos estándar a componentes personalizados de TailwindCSS para que las listas (\texttt{ul/ol}), los enlaces (\texttt{a}) y las negritas (\texttt{strong}) mantengan un estilo visual premium y uniforme.
...
%Uno de los resultados que más me sorprendió fue la fiabilidad del sistema a la hora de clasificar las intenciones del usuario. Al asignar el rol de enrutador a Llama 3.1 8B con una temperatura de 0.0, conseguí decisiones prácticamente deterministas en milisegundos. El nodo de rechazo, en particular, funcionó de forma especialmente consistente, interceptando sin problemas las preguntas que se salían de la temática universitaria.
