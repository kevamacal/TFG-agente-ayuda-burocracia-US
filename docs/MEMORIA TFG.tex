...
Como marco de trabajo ágil se adoptó Scrum, aunque evidentemente hubo que adaptarlo a la realidad de un TFG individual. No tiene mucho sentido hacer una \textit{daily} contigo mismo, pero sí que resultó muy útil mantener la filosofía de sprints cortos, backlog priorizado y revisiones periódicas.

\subsection{Roles}

Al ser un proyecto individual, los roles de Scrum se han repartido de la manera más lógica posible:
\begin{itemize}
    \item \textbf{Product Owner:} Los tutores del TFG (José Enrique Sánchez López y Pablo Reina Jiménez), encargados de definir prioridades y validar que lo desarrollado tenía sentido.
    \item \textbf{Scrum Master y Equipo de Desarrollo:} Yo mismo, Kevin Amador Calzadilla, que gestionaba el backlog, ejecutaba los sprints y hacía las retrospectivas (básicamente, pararme a pensar qué había ido bien y qué podía mejorar).
\end{itemize}

\subsection{Artefactos}

\begin{itemize}
    \item \textbf{Product Backlog:} Lista priorizada de funcionalidades y tareas técnicas, mantenida y actualizada de forma continua a lo largo del desarrollo.
    \item \textbf{Sprint Backlog:} Subconjunto de tareas seleccionadas para cada sprint de dos semanas.
    \item \textbf{Incremento:} Cada sprint concluye con una versión funcional del sistema que integra las nuevas funcionalidades desarrolladas.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Control de versiones:} Git con repositorio alojado en GitHub.
    \item \textbf{Gestión de tareas:} Seguimiento manual del backlog y los sprints mediante documentación propia del alumno.
    \item \textbf{Entorno de desarrollo:} Visual Studio Code con extensiones para Python, LaTeX y Docker.
    \item \textbf{Contenedorización:} Docker y Docker Compose para la reproducibilidad del entorno.
\end{itemize}

\chapter{Planificación}
\section{Definición de sprints}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}clp{8cm}@{}}
        \toprule
        \textbf{Sprint} & \textbf{Periodo} & \textbf{Hito principal}                                      \\
        \midrule
...
\textit{Nota: Inserta aquí la imagen de tu diagrama de Gantt. Puedes usar el siguiente bloque como plantilla:}

\begin{figure}[H]
    \centering
    % Descomentar y ajustar el nombre del archivo cuando tengas la imagen
...
\begin{itemize}
    \item Investigación y estado del arte: 15\%
    \item Diseño e implementación del pipeline RAG: 25\%
    \item Desarrollo del agente (LangGraph): 20\%
    \item Backend y frontend: 15\%
    \item Evaluación y validación: 10\%
    \item Documentación y memoria: 15\%
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{Matriz de análisis de riesgos del proyecto}
    \begin{tabular}{@{}p{4.5cm}ccp{5.5cm}@{}}
        \toprule
        \textbf{Riesgo}                                       & \textbf{Probabilidad} & \textbf{Impacto} & \textbf{Mitigación}                                             \\
        \midrule
        Límite de tokens en APIs gratuitas (Groq, LlamaParse) & Alta                  & Alto             & Estrategia multi-modelo con modelos ligeros para tareas simples \\
...
\begin{itemize}
    \item \textbf{Desarrollador Junior:} 400 horas $\times$ 15 €/hora = 6.000 €
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lr@{}}
        ...
        \midrule
        \textbf{Base Imponible}           & \textbf{6.225,00}           \\
        IVA (21\%)                        & 1.307,25                    \\
        \midrule
        \textbf{Total Presupuesto}        & \textbf{7.532,25}           \\
        \bottomrule
    \end{tabular}
...
\begin{itemize}
    \item \textbf{RAG de conocimiento normativo:} La base de datos vectorial en Pinecone contiene los fragmentos de los 15 documentos oficiales de la US (normativas de matrícula, baremos de contratación, calendarios académicos, guías de viajes, etc.).
...
\begin{itemize}
    \item \textbf{Ingesta semántica:} Se ha desarrollado un pipeline capaz de procesar 15 documentos oficiales de la US, preservando tablas, jerarquías y notas al pie mediante LlamaParse.
    ...
    \item \textbf{Validación:} Se ha evaluado el sistema con 34 casos de prueba utilizando el paradigma LLM-as-Judge.
\end{itemize}
...
\textit{Nota: Pega aquí el contenido de tu archivo \texttt{templates.py} utilizando el entorno \texttt{lstlisting} para formatearlo como código.}
